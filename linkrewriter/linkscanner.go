...
func createScanner(origin *url.URL) LinkScanner {
...
func (_ *linkScannerImpl) Scan(responseBody string, response *http.Response) []*ScanResult {
	mimeType := strings.ToLower(mimeFromResponse(response))
	...
	for _, regexp := range regexps {
		capturedUrls := scanWith(regexp, responseBody, response.Request.URL)
